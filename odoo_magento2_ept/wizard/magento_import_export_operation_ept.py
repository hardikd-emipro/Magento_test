...
class MagentoImportExportEpt(models.TransientModel):
    ...
    operations = fields.Selection([
        ...
        ('import_sale_order', 'Import Sale Orders'),
    ...
    def import_customer_operation(self, instances):
        ...
        for instance in instances:
            ...
            customer_queue_data = customer_data_queue_obj.magento_create_customer_data_queues(**kwargs)
        result = self.return_customer_queue_form_or_tree_view(customer_queue_data)
        return result

    def return_customer_queue_form_or_tree_view(self, customer_queue_data):
        """
        it's return the tree view or form view based on the total_customer_queues.
        :param customer_queue_data: {'customer_queue': magento.customer.data.queue.ept(X,), 'count': X, 'total_customer_queues': X}
        :return: view with domain
        """
        result = {
            'name': _('Magento Customer Data Queue'),
            'res_model': 'magento.customer.data.queue.ept',
            'type': IR_ACTION_ACT_WINDOW,
        }
        if customer_queue_data.get('total_customer_queues') == 1:
            view_ref = self.env[IR_MODEL_DATA].get_object_reference(
                'odoo_magento2_ept', 'view_magento_customer_data_queue_ept_form'
            )
            ...
            result.update({
                ...
                'view_mode': 'form',
                'view_id': view_id,
                'res_id': customer_queue_data.get('customer_queue').id,
                'target': 'current'
            })
        else:
            result.update({
    ...
    def import_product_stock_operation(self, instances):
        ...
        for instance in instances:
            ...
            if instance.magento_version in ['2.1', '2.2'] or not instance.is_multi_warehouse_in_magento:
                is_log_exist = magento_product_product.create_product_inventory(instance, self.auto_validate_stock)
            else:
                inventory_locations = magento_inventory_locations_obj.search([
                    ('magento_instance_id', '=', instance.id)])
                magento_product_product.create_product_multi_inventory(instance, inventory_locations, self.auto_validate_stock)
        if is_log_exist:
            result = {
