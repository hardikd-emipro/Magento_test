...
class ImageUrl(http.Controller):
    ...
    @http.route('/app_notification/receive', type='json', method='POST', auth='public')
    def get_in_app_update_notification(self):
        """
        This controller are used to create/update the emipro application update details in the
        customer's database.
        ...
        try:
            ...
            _logger.error(error)
        if data:
            request.env['emipro.app.version.details'].create_update_details(data)
        return True

    @http.route('/app_notification/get_module', type='json', method='POST', auth='public')
    def receive_in_app_update_notification(self):
        """
        ...
        if is_notify:
            ...
            if params:
                ...
                app_version = request.env['emipro.app.version.details']
                author = app_version.get_author()
                module = module.search([('name', '=', module_name), ('author', 'in', author)], limit=1)
                if module:
                    updates = app_version.find_update_details(module)
                    update_url = [update.get('update_url') for update in updates if update.get('update_url')]
                    details.update({
                        'updates': updates,
    ...
    def deny_update(self):
        ...
        if params:
            module_name = params.get('module_name')
            app_version = request.env['emipro.app.version.details']
            app_version.disable_notification(module_name)
        return True
